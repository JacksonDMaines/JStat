...
LetXbe the maximum of two rolls of a four-sided die. The sample space:
...
Definition:The CDF of a discrete random variableXis:
...
Step function:F(x) =P[X≤x], whereX=max(two 4-sides dices)

### 2.3 Recovering PMF from CDF

Letx 1 < x 2 < x 3 <··· be the possible values ofX.
...
Definition:LetXbe a discrete random variable with probability mass function (PMF)f(x) =  
P[X=x]. The expected value (or mean) of X, denoted byE[X] orμ, is defined as: provided

that the sum exists.

- x: a possible value of the random variable.
- f(x): the probability thatX=x.
- The expected value is a of the possible values ofX, weighted  
    by their probabilities.

Interpretation:If the experiment is repeated many times, the average value ofXwill approach  
E[X].

Example: LetXbe the outcome of rolling a fair six-sided die. Possible values:x= 1, 2 , 3 , 4 , 5 ,6.  
Each has probability:P(X=x) =^16

Example: Let a random variableXhave the following PMF:P(X= 1) = 0. 2 , P(X= 2) =
...
LetXandY be random variables, and letaandbbe constants. Then:

3.1.2 Function of a Random Variable

Letg(X) be a real-valued function of a discrete random variableX. Then:

You can compute the expectation of any function ofX, such asE[X^2 ],E[lnX], etc.
...
LetH=∑^100 i=1Xi: total number of heads

Step 1:Compute expected value of eachXi

Step 2:Use linearity of expectation for the functionH=∑^100 i=1Xi

Example: LetXbe a random variable with known expected value:
...
Definition: LetXbe a discrete random variable with expected valueμ=E[X]. The variance  
ofX, denoted by Var(X) orσ^2 , is defined as:

- It measures the spread or dispersion of the values ofXaround the mean.
...
LetY=aX+b, wherea,b∈R, andXis a random variable.
...
- Scaling byastretches or compresses variability by a factor ofa^2.
...
LetXandY be independent random variables. Then:
